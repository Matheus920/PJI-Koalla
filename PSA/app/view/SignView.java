...
public class SignView {
    ...
    private void setFields() {
        ...
        MaskField.maxLength(tfName, 255);
        tfEmail.setFont(Font.font("Segoe UI", 18));
        MaskField.maxLength(tfEmail, 255);
        tfFile.setFont(Font.font("Segoe UI", 18));
        MaskField.maxLength(tfFile, 255);
        tfInstitution.setFont(Font.font("Segoe UI", 18));
        MaskField.maxLength(tfInstitution, 255);
        pfPassword.setFont(Font.font("Segoe UI", 18));
        MaskField.maxLength(pfPassword, 255);
        pfConfirm.setFont(Font.font("Segoe UI", 18));
        MaskField.maxLength(pfConfirm, 255);
    ...
    private void setButtons() {
        ...
        sign.setOnAction(e->{
            ...
            if(lblOk.getText().equals("")) {
                Login login = new Login(tfEmail.getText(), pfPassword.getText(), 1);
                new LoginController().addLogin(login);
                User user = new User(tfName.getText(), login, tfInstitution.getText(), dob.getValue(), tfFile.getText(), false);
                User userWithId = new CRUDUser().addUser(user);
                Main.getPrivilege().setPrivilegeType(PrivilegeTypeInterface.USER);
                Main.setUser(userWithId);
                Main.refresh();
                stage.close();
            }
        });
